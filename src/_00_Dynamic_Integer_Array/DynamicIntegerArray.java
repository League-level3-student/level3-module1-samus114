...
public class DynamicIntegerArray {
	...
	public void add(int v) {
		...
		// C. iterate through the member array and copy every element from the member
		// array
		// to the new array
...
//done :)
	...
	public void remove(int location) {
		...
		for (int i = 0; i < arr.length; i++) {
			// C. if i is less than location set the element at i of the new array to the
			// element at i of the member array
			if (i < location) {
				array[i] = arr[i];
				// D. else if i is greater than location set the element at i - 1 of the new
				// array to the element at i of the member array
			} else if (i > location) {
				array[i - 1] = arr[i];
				// E. else, continue;
			} else {
